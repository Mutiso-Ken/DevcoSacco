...
table 50230 "Loans Register"
{
    ...
    fields
    {
        ...
        field(4; "Loan Product Type"; Code[20])
        {
            ...
            begin
                ...
                if LoanApp.FindFirst() then begin
                    if Dialog.Confirm('Member already has an existing %1 loan. Do you want to proceed?', false, "Loan Product Type") then begin
                    end else begin
                        Error('The Member has an Existing Loan with the same product Type. Please Change the Product Type');
                    end;
                end;
                ...
                if LoanType.Get("Loan Product Type") then begin
                    ...
                    if (LoanType."Loan Product Expiry Date" = 0D) or (LoanType."Loan Product Expiry Date" > Today) then begin
                        ...
                        //Check if Member has Existing Loan
                        // Proceed with next steps here

                        if LoanType.Get("Loan Product Type") then begin
                            "Loan Product Type Name" := LoanType."Product Description";
                            Interest := LoanType."Interest rate";
                            Mulitiplier := LoanType."Deposits Multiplier"; // Fixed typo from 'Mulitiplier'
                            "Deposits Mulitiplier" := (Shares * Mulitiplier); // Fixed 'sHARES' typo
                            ...
                            Rec.Modify();
                            if not LoanType."Use Cycles" then begin
                                "Loan Cycle" := 0;
                                "Max. Installments" := LoanType."No of Installment";
                                "Max. Loan Amount" := LoanType."Max. Loan Amount";
                                Installments := LoanType."Default Installements";
        ...
        field(68068; "1st Notice"; Boolean)
        {
        }
        field(68069; "2nd Notice"; Boolean)
        {
        }
        field(68070; "Final Notice"; Boolean)
        {
        }
        ...
        field(515163002; PortalGuaranteedBoolean; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(515163003; "Bank Account No"; Integer)
        {
            DataClassification = ToBeClassified;
        }
        field(515163004; "Bank Account Name"; Text[2048])
        {
            DataClassification = ToBeClassified;
        }
        field(515163005; "Mpesa Number"; Code[13])
        {
            DataClassification = ToBeClassified;
        }
        field(515163006; "Is Loan Top Up"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
    }
    ...
    begin
        ...
        //Delete Loan Offset Details
        LoanOffset.reset;
        LoanOffset.SetRange("Loan No.", Rec."Loan  No.");
        if LoanOffset.FindSet() then begin
            LoanOffset.DeleteAll();
        end;
    end;
