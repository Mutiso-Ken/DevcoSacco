...
Page 50215 "Guarantor Sub Card"
{
        ...
        area(navigation)
        {
            group(Approvals)
            {
                ...
                action("Approval Request")
                {
                    ...
                    trigger OnAction()
                    var
                        text001: label 'This batch is already approved';
                        SrestepApprovalsCodeUnit: Codeunit SurestepApprovalsCodeUnit;
                    begin
                        if rec."Created By" <> UserId then begin
                            if Rec.Status = Rec.Status::Approved then
                                Error(text001);
                            //Approval here
                            if Confirm('Approval Request?', false) = true then begin
                ...
                action("Process Substitution")
                {
                    ...
                    begin
                        ...
                        LGuarantor.Reset;
                        LGuarantor.SetRange("Loan No", Rec."Loan Guaranteed");
                        LGuarantor.SetRange("Member No", Rec."Substituting Member");
                        ...
                        if LGuarantor.FindSet then begin
                            ...
                            if GSubLine.FindSet then begin
                                repeat
                                    TotalReplaced += GSubLine."Sub Amount Guaranteed";
                                until GSubLine.Next = 0;
                            end;
                            ...
                            if GSubLine.FindSet then begin
                                repeat
                                    AmountToAssign :=
                                        CalculateAmountGuaranteed(
                                            GSubLine."Sub Amount Guaranteed",
                                            TotalReplaced,
                                            GSubLine."Amount Guaranteed");

                                    LoanGuar.Reset();
                                    LoanGuar.SetRange("Loan No", Rec."Loan Guaranteed");
                                    LoanGuar.SetRange("Member No", GSubLine."Substitute Member");

                                    if LoanGuar.FindFirst() then begin
                                        LoanGuar."Amont Guaranteed" :=
                                            LoanGuar."Amont Guaranteed" + AmountToAssign;
                                        ...
                                        NewLGuar."Amont Guaranteed" := AmountToAssign;
                    ...
                    begin
                        ...
                        if LGuarantor.FindSet then begin
                            ...
                            if GSubLine.FindSet then begin
                                repeat
                                    LoanGuar.Reset();
                                    LoanGuar.SetRange(LoanGuar."Loan No", GSubLine."Loan No.");
                                    LoanGuar.SetRange(LoanGuar."Member No", GSubLine."Member No");
                                    if not LoanGuar.FindSet() then begin
                                        NewLGuar.Init;
                                        NewLGuar."Loan No" := Rec."Loan Guaranteed";
                                        NewLGuar."Member No" := GSubLine."Substitute Member";
                                        NewLGuar.Validate(NewLGuar."Member No");
                                        ...
                                        NewLGuar.Validate(NewLGuar."Member No");
                                        NewLGuar.Name := GSubLine."Substitute Member Name";
                                        NewLGuar."Amont Guaranteed" := CalculateAmountGuaranteed(GSubLine."Sub Amount Guaranteed", TotalReplaced, GSubLine."Amount Guaranteed") + LoanGuar."Amont Guaranteed";
                                        NewLGuar."Substituted Guarantor" := GSubLine."Member No";
                                        NewLGuar.Shares := GSubLine."Substitute Member Deposits";
                                        NewLGuar.Modify();
                                    end;

                                    OldGuar := GSubLine."Member No";
                                    SubMember := GSubLine."Substitute Member";

                                    SmsBody := 'Dear ' + GSubLine."Member Name" +
                                               ', you have been replaced as guarantor by ' +
                                               GSubLine."Substitute Member Name" + '.';
                                    Saccogensetup.FnSendSMS(
                                        'GUARANTOR-REPLACED',
                                        SmsBody,
                                        OldGuar,
                                        GetPhoneNumber(OldGuar)
                                    );

                                    SmsBody := 'Dear ' + GSubLine."Substitute Member Name" +
                                               ', you have replaced ' + GSubLine."Member Name" +
                                               ' as guarantor for loan ' + GSubLine."Loan No." + '.';
                                    Saccogensetup.FnSendSMS(
                                        'GUARANTOR-NEW',
                                        SmsBody,
                                        SubMember,
                                        GetPhoneNumber(SubMember)
                                    );
    ...
    local procedure GetPhoneNumber(MemberNo: Code[20]): Text
    begin
        CustRecord.Reset();
