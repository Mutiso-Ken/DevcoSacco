Page 50491 "Overdraft Applications Card"
{
    //DeleteAllowed = false;
    ...
    actions
    {
        area(navigation)
        {
                        ...
                        if Confirm('Are you sure you want to POST Overdtaft Loan amount Ksh.' + Format(Rec."Approved Amount") + ' to Loanee?', false) = false then begin
                            ...
                            if GenJournalLine.Find('-') then begin
                                if Codeunit.Run(Codeunit::"Gen. Jnl.-Post Sacco", GenJournalLine) = true then begin
                                    ...
                                    Error('There was an error in posting the loan,please contact the system administrator');
                                end;
            ...
            group(OffsetLoans)
            {
                ...
                {
                    ApplicationArea = Basic;
                    Caption = 'Offset Overdrafts';
                    ToolTip = 'Off set Member Previous Overdraft Loans';
                    Enabled = true;
                    Image = Loaner;
                    Promoted = true;
                    PromotedCategory = Process;
