...
Page 50245 "Loan Application Card"
{
    DeleteAllowed = true;
    Editable = true;
    PageType = Card;
        ...
        area(content)
        {
            group(General)
            {
                        ...
                        if Rec."Deboost Loan Applied" = true then begin
                            begin
                                ...
                                if (Rec."Member Deposits" * Rec.Mulitiplier) < ((rec."Requested Amount" + Rec."Existing Loan") - OffesetAmount) then begin
                                    Rec."Deboost Amount" := ((((rec."Requested Amount" + (Rec."Existing Loan" - OffesetAmount))) - (Rec."Member Deposits" * Rec.Mulitiplier)) / Rec.Mulitiplier);
                ...
                field("Approval Status"; Rec."Approval Status")
                {
                    ...
                    trigger OnValidate()
                    begin
                        // Sync Loan Status with Approval Status
                        if "Approval Status" = "Approval Status"::Approved then
                            "Loan Status" := "Loan Status"::Approved
                        else if "Approval Status" = "Approval Status"::Pending then
                            "Loan Status" := "Loan Status"::Appraisal
                        else if "Approval Status" = "Approval Status"::Rejected then
                ...
                field("Bank Name"; Rec."Bank Name")
                {
                    ApplicationArea = Basic;
                    Editable = MNoEditable;
                    ShowMandatory = isBankChosen;
                    Visible = isBankChosen;
                }
                ...
                field("Mpesa Number"; Rec."Mpesa Number")
                {
                    ApplicationArea = Basic;
                    Editable = MNoEditable;
                    ShowMandatory = isMpesaChosen;
                    Visible = isMpesaChosen;
                }
        ...
        area(navigation)
        {
            group(Loan)
            {
                ...
                action("Send Approvals")
                {
                    ...
                    begin
                        ...
                        if Confirm('Send Approval Request For Loan Application of Ksh. ' + Format(Rec."Approved Amount") + ' applied by ' + Format(Rec."Client Name") + ' ?', false) = false then begin
                            ...
                            "Loan Status" := "Loan Status"::Appraisal; // Add this line
                            Modify();
                ...
                action("Cancel Approvals")
                {
                    ...
                    begin
                        if Confirm('Cancel Approval?', false) = false then begin
                            ...
                            "Loan Status" := "Loan Status"::Application; // Add this line
                            Modify();
                ...
                action(Attachments)
                {
                    ...
                    ToolTip = 'View attached images as well as documents.';
                }
    ...
    var
        isMpesaChosen: Boolean;
        isBankChosen: Boolean;
        ...
        NotDividendAdvance: Boolean;

    local procedure SetDisbursementVisibility()
    begin
        isBankChosen := Rec."Mode of Disbursement" = Rec."Mode of Disbursement"::BANK;
        isMpesaChosen := Rec."Mode of Disbursement" = Rec."Mode of Disbursement"::MPESA;
        CurrPage.Update();
    end;

    procedure UpdateControl()
    begin
        If Rec."Loan Product Type" = 'DIVIDEND ADVANCE' then Begin
            NotDividendAdvance := false
        end else begin
            NotDividendAdvance := true;
        end;
    ...
    begin
        ...
        if (LoanType.get("Loan Product Type")) then begin
            if LoanType."Charge Interest Upfront" = false then begin
...
'Congratulations';
            ...
            REPEAT
                ...
                IF Cust.FIND('-') THEN BEGIN
                    ...
                    IF LoanApp.GET(LoanGuar."Loan No") THEN
                        ...
                        + ' to ' + Rec."Client Name" + '  ' +
                        'Loan Type ' + Rec."Loan Product Type" + ' ' + 'of ' + FORMAT(Rec."Requested Amount") + ' at Devco Sacco Ltd. Call 0721470786 if in dispute';
                    ;
    ...
    procedure SyncLoanAndApprovalStatus()
    begin
