Page 50256 "Loan Disburesment Batch Card"
{
    DeleteAllowed = false;
    PageType = Card;
    ...
    begin
        ...
        IF LoanApps.GET(VarLoanNo) THEN BEGIN
            repeat
                ...
                VarAmounttoDisburse := VarAmounttoDisburse - Round(LoanApps."Deboost Commision", 0.01, '=');
                //Debosting commsion
                LineNo := LineNo + 10000;
                ...
                VarAmounttoDisburse := VarAmounttoDisburse - Round(LoanApps."Deboost Amount", 0.01, '=');
                //Loan Interest upfront



                If LoanType."Charge Interest Upfront" = true then begin
                    Interestupfront := Round((LoanApps."Recommended Amount" * (LoanType."Interest rate" / 100)), 0.01, '=');
                    LineNo := LineNo + 10000;
                    SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, LoanApps."Loan  No.", LineNo, GenJournalLine."Transaction Type"::" ", GenJournalLine."Account Type"::"G/L Account", LoanType."Loan Interest Account", DirbursementDate, Round(Interestupfront, 0.01, '=') * -1, 'BOSA', Rec."Batch No.",
                    'Interest Paid Dividend Advance' + Format(LoanApps."Loan  No."), ' ');
                    VarAmounttoDisburse := VarAmounttoDisburse - Round(Interestupfront, 0.01, '=');
                end;


                //End of Interest upfront
                //..Legal Fees
                LineNo := LineNo + 10000;
                ...
                VarAmounttoDisburse := VarAmounttoDisburse - Round(LoanApps."Legal Cost", 0.01, '=');
