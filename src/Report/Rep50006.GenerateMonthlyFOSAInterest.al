                    ...
                    IF AccountType."Earns Interest" = TRUE THEN BEGIN
                        REPEAT
                            ...
                            IF Account.FIND('-') THEN BEGIN
                                ...
                                Bal := Account."FOSA Balance";
                                Account."Not Qualify for Interest" := FALSE;
                                IF Account."FOSA Balance" <= AccountType."Interest Calc Min Balance" THEN BEGIN
                                    Account."Not Qualify for Interest" := TRUE;
                                    Account.MODIFY;
                                END
                                ELSE
                                    IF ((Account."FOSA Balance" >= AccountType."Interest Calc Min Balance") AND (NOT Account."Not Qualify for Interest")) THEN BEGIN
                                        IF AccountType."Fixed Deposit" = TRUE THEN BEGIN
                                            ...
                                            IF FDInterestCalc.FIND('-') THEN BEGIN
                                                REPEAT
                                                    IF (FDInterestCalc."Minimum Amount" <= Account."FOSA Balance") AND (Account."FOSA Balance" <= FDInterestCalc."Maximum Amount") THEN
                                        ...
                                        IF AccountType."Fixed Deposit" = FALSE THEN BEGIN
                                            IF ((Account."FOSA Balance" >= AccountType."Interest Calc Min Balance") AND (NOT Account."Not Qualify for Interest")) THEN BEGIN
                                                AccruedInt := AccruedInt + ((IntRate / 36500) * Account."FOSA Balance");
                                            END;
                                        END;
                                    END;
                            END
