...
Report 50042 "Generate Member Salaries Batch"
{
    ProcessingOnly = true;
    dataset
    {
        dataitem("Periodics Processing Lines"; "Periodics Processing Lines")
        {
                ...
                IF FundsUser.GET(USERID) THEN BEGIN
                    ...
                    IF DontChargeProcFee = FALSE THEN BEGIN
                        IF Charges.GET('SAL') THEN BEGIN
                            ...
                            ExciseFee := SalFee * (SaccoGenSetUp."Excise Duty(%)" / 100);
                        END;
                    END;
                END;
            end;
            ...
            begin
                ...
                if PeriodicProcessingLines.Find('-') then begin
                    repeat
                        DialogBox.Open('Processing Lines');
                        // if PeriodicProcessingLines."Account Is Blocked" = true then begin
                        //     Error('There is a blocked account %1 kindly unblock before proceeding', PeriodicProcessingLines."Account No");
                        // end;
                        //.......................Insert entries
                        if VendorTable.get(PeriodicProcessingLines."Account No") then begin
                            ...
                            NetAmountPaidToMember := FnProcessMemberEntriesToGL(PeriodicProcessingLines.Amount, PeriodicProcessingLines."Account No", PostingDocumentNo, PostingDate, DActivity, DBranch);
                            ...
                            //AvailableBal := NetAmountPaidToMember;//2030
                            //...............
                            ...
                            if BalanceAfterPrincipleRecovery > 0 then begin
                                BalanceAfterPrincipleRecovery := BalanceAfterPrincipleRecovery;
    ...
    local procedure FnProcessMemberEntriesToGL(Amount: Decimal; AccountNo: Code[100]; PostingDocumentNo: Code[100]; PostingDate: Date; DActivity: Code[50]; DBranch: Code[50]): Decimal
    var
        NetPay: Decimal;
    begin
        NetPay := 0;
        ...
        GenJournalLine.Description := 'Salary Processed ' + Format(PostingDocumentNo);
        ...
        GenJournalLine."Account No." := AccountNo;
        GenJournalLine.VALIDATE(GenJournalLine."Account No.");
        GenJournalLine.Description := 'Salary Processing Fee ' + Format(PostingDocumentNo);
    ...
    begin
        ...
        LoansRegister.SetRange(LoansRegister."Recovery Mode", LoansRegister."Recovery Mode"::Salary);
    ...
    begin
        ...
        //........................
        InterestToRecover := ExpectedInterestToDate - PaidAmount;
    ...
    begin
        ...
        StandingOrders.SetRange(StandingOrders."Income Type", StandingOrders."Income Type"::Salary);
        ...
        if LoansRegister.Find('-') then begin
            repeat
                if MemberBalanceAfterPrinciplePay > 0 then begin
                    if LoansRegister."Outstanding Balance" > 0 then begin
                        PrincipleToRecover := 0;
                        if LoansRegister."Loan Disbursement Date" <= 20230807D THEN begin
                            PrincipleToRecover := LoansRegister."Outstanding Balance";
                        end else
                            if (LoansRegister."Loan Disbursement Date" > 20230807D) and (LoansRegister."Recovery Mode" = LoansRegister."Recovery Mode"::Salary) THEN begin
                                PrincipleToRecover := LoansRegister."Loan Principle Repayment";
                            end;
                        //********Cr Loan Account
                        LineNo := LineNo + 10000;
