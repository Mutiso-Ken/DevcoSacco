            ...
            begin
                ...
                if GLAccount.FindSet then begin
                    ...
                    repeat
                        ...
                        if GLEntry.FindSet then begin
                            GLEntry.CalcSums(Amount);
                            DividendPayable += -1 * GLEntry.Amount;
                        end;
                ...
                Taxes := Taxes - Taxpaid;
                Dividends := (ShareCapital * SaccoGen."Interest on Share Capital(%)") * 0.01;
                InterestOndeposits := (Nonwithdrawabledeposits * SaccoGen."Interest On Current Shares") * 0.01;
                ProposedDividends := Dividends + InterestOndeposits;
                // ProposedHonoraria := (((Nonwithdrawabledeposits * SaccoGen."Interest On Current Shares") * 0.01) * (SaccoGen."Proposed Honoraria" * 0.01));

                CurrentYearSurplus := ((ProposedDividends + Taxes) + surplus);

                // StatturyAdjustment := CurrentYearSurplus * 0.20;
